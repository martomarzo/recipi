...
- Secciones: `src/app/dietas` (timeline, gantt en `/disponibilidad`, resumen+PDF en `/resumen`), `src/app/recetas` (import por link en `/importar`, pipeline en `src/lib/recetas/`; ingredientes faltantes se crean con macros estimados por IA vía `/api/ingredientes/estimar`, modelo barato con fallback en `anthropicModel.ts`), `src/app/ingredientes`.
- Auth: `src/lib/auth.ts` (sesiones cookie + invitaciones); dietas scopeadas por acceso en `src/lib/dietAccess.ts` — dueño o share `DietShare` (viewer/editor); sin acceso = 404. Gestión de usuarios en `/usuarios` (alta directa + edición + borrado —nunca a uno mismo— + link de invitación, sin roles).
